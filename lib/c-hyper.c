...
CURLcode Curl_http(struct connectdata *conn, bool *done)
{
  ...
  *done = TRUE;
  ...
  /* hyper_headers_add() and _set() return 'void' but really shouldn't */
  hyper_headers_set(headers, (uint8_t *)"Host", 4,
  ...
  if(HYPER_OK != hyper_executor_push(h->exec, sendtask)) {
  ...
  */
  /*
  if(h->handshake)
    hyper_clientconn_handshake_free(h->handshake);
  */
